...
(function (factory) {
  ...
  DraggableGridList.prototype = {
    ...
    _onStop: function(event, ui) {
      ...
      item = this._getItemByElement(ui.helper);
      // TODO should use layout function
      if(item.x > 0) {
        $('#grid').gridList('resizeItem', ui.helper[0], {
          w: 2,
          h: item.h
        });
      } else {
    ...
    _getItemWidth: function(item) {
      // TODO should use layout function
      return item.x > 0 ? 2 * this._cellWidth: this._cellWidth;
    },
    ...
    _applySizeToItems: function() {
      for (var i = 0; i < this.items.length; i++) {
        this.items[i].$element.css({
          // TODO  should use layout function
          width: this.items[i].x > 0 ? 2 * this._cellWidth : 1 * this._cellWidth,
    ...
    _applyPositionToItems: function() {
      ...
      for (var i = 0; i < this.items.length; i++) {
        ...
        this.items[i].$element.css({
          left: this.items[i].x > 0 ? 1 * this._cellWidth : this.items[i].x * this._cellWidth,
